...
class DEBLUR(object):
    ...
    def generator(self, inputs, inputs_render, coeff, reuse=False, scope='g_net'):
        ...
        # pre-handle coeff
        def pad_coeff(coeff_p, n_n, h_h, w_w, i):
           # tf.reset_default_graph()
            coeff_p = tf.reshape(coeff_p,[n_n, 81])
            # coeff = tf.expand_dims(coeff, axis=0)
            coeff_p = tf.cast(coeff_p,tf.float32)
            name = 'Fc_'+str(i)
            coeff_p = tf.layers.dense(inputs=coeff_p, units=h_h*w_w, activation=None)
            coeff_p = tf.reshape(coeff_p, [n_n, h_h, w_w])
            coeff_p = tf.expand_dims(coeff_p,axis=3)



            # h_r = int(round((h_h-9) * 0.5))
            # w_r = int(round((w_w-9) * 0.5))
            # coeff_pm = tf.pad(coeff_p, [[h_r, h_h-9-h_r],[w_r, w_w-9-w_r]])
            # print(coeff_p.get_shape())
            # hwc = h_h*w_w
            # coeff = tf.cast(coeff,tf.float32)
            # coeff_p = tf.reshape(coeff_p,[1,81])
            # coeff_pm = tf.layers.dense(inputs=coeff_p, units=hwc, activation=None, reuse=tf.AUTO_REUSE)
            # print(coeff_pm.get_shape())
            # coeff_pm = tf.reshape(tf.squeeze(coeff_pm), [h_h,w_w])
            # coeff_pm = tf.expand_dims(coeff_pm, 0)
            # from 1's to 16's at axis=0
            # for x in xrange(0, int(np.log2(self.batch_size))):
            #     coeff_pm = tf.concat([coeff_pm, coeff_pm], axis=0)
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                for i in xrange(self.n_levels):
                    ...
                    if self.args.face == 'coeff' or self.args.face == 'both':
                        ...
                        # coeff_m = pad_coeff(coeff, n_c, h_c, w_c, i)
